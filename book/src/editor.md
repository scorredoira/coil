...
The file tree is the sidebar on the left listing the workspace, the one `<space>T` focuses. It follows the focused buffer, expanding its directory and marking the file. Inside it, `j`/`k` move, `l`/`Enter` expand a directory or open a file, `h` collapses or jumps to the parent, `H` collapses everything, `R` reloads, `Esc` returns to the editor and `q` hides the tree (`<space>t` toggles it from the editor). `a` creates a file, or a directory when the name ends in `/`, next to the cursor; `r` renames the entry under the cursor, moving its open buffers along; `d` deletes it after confirmation. A click opens or expands the row under the mouse. `Tab`, or a click on a tab's name, switches between three tabs: Files, the whole workspace; Changes, only what `git status` names — each file changed, added, deleted or renamed since the last commit, staged or not, with its letter (`M`, `A`, `D`, `R`) and a directory holding one directory folded into one row; and Commits, the history, newest first, each commit with its short hash, its subject and its age, the older ones read as the cursor nears the end. `Enter` on a commit lists the files it touched the way Changes does, under a row standing for the whole commit, and the focused view shows its diff in a scratch buffer, following the cursor: the whole commit on its row, a directory's files on a directory, one file on a file. `Enter` on a file goes over to read that diff, and `Esc` goes back from the files to the history. Changes and Commits ask git again every two seconds while they are on screen, and `R` asks at once; creating, renaming and deleting do nothing in Commits, which is the history and not the disk. What the Files tab lists follows the `[editor.file-explorer]` options and nothing else: the helix ignore files narrow what a search or a picker walks, never what the tree shows.
